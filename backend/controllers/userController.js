...
export const loginThroughusername= async(req,res)=>{
    ...
    try {
        ...
        if (isMatch) {
            // Generate JWT token
            const token = generateToken(user.dataValues.row_id);
...
export const fetchDetails = async(req,res)=>{
    ...
    console.log(user)
    const userName = await userInfo.findOne({
        where : {
            row_id: user.id
        }
    })
    ...
    if(userName){
        const username = userName.dataValues.username||userName.dataValues.username
        console.log(username)
